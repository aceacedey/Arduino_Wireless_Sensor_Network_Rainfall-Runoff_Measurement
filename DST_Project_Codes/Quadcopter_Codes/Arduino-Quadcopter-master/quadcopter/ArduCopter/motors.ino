...
static void arm_motors_check()
{
    ...
    //cliSerial->printf_P(PSTR("\nG_RC_CONTROL : %f"),(float)g.rc_3.control_in);
    //cliSerial->printf_P(PSTR("\nG_RC3_DEADZONE : %f"),(float)g.rc_3._dead_zone); 	
    // ensure throttle is down
    if (g.rc_3.control_in > 0) { // 137 was 0 previously
       // char buf[12];
       // cliSerial->printf_P(itoa(g.rc_3.control_in,buf,10));
        arming_counter = 0;
        return;
    }
