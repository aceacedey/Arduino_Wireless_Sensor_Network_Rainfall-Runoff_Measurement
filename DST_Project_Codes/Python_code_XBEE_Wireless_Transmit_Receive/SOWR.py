...
startpath = 'C:\Program Files\Apache Software Foundation\Apache2.2\htdocs\mapiitkgp.com\Data\SOWR'
...
while var == 1:
    if int(cmonth) == int(omonth):
        if int(cday) == int(oday):
            ...
            print(f_dir)
            file = open(os.path.join(f_dir, "SOWR_%s.txt" %oday), "w")
            try:
                req = Request(('http://10.172.3.202'), headers={'User-Agent': 'Mozilla/5.0'})
        ...
        newpath = 'C:\Program Files\Apache Software Foundation\Apache2.2\htdocs\mapiitkgp.com\Data\SOWR'
