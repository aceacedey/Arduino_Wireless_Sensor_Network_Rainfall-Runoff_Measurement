...
startpath = 'C:\Program Files\Apache Software Foundation\Apache2.2\htdocs\mapiitkgp.com\Data\Civil'
...
while var == 1:
    if int(cmonth) == int(omonth):
        if int(cday) == int(oday):
            ...
            #print(f_dir)
            file = open(os.path.join(f_dir, "Civil_%s.txt" %oday), "w")
            try:
                req = Request(('http://10.19.2.42'), headers={'User-Agent': 'Mozilla/5.0'})
                ...
                for i, line in enumerate (gtindex.splitlines()):
                    if line == (" RAINFALL in past 5 mintutes : "):
        ...
        newpath = 'C:\Program Files\Apache Software Foundation\Apache2.2\htdocs\mapiitkgp.com\Data\Civil'
